...
Elm.Mechanics = Elm.Mechanics || {};
Elm.Mechanics.make = function (_elm) {
   "use strict";
   _elm.Mechanics = _elm.Mechanics || {};
   if (_elm.Mechanics.values) return _elm.Mechanics.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var stateDerivative = F2(function (accels,state) {
      return _U.update(state,
      {time: 1
      ,particles: A2($Dict.map,
      F2(function (key,particle) {
         var _p0 = A2($Dict.get,key,accels);
         if (_p0.ctor === "Just") {
               return _U.update(particle,{position: particle.velocity,velocity: A2(_p0._0,particle,state)});
            } else {
               return particle;
            }
      }),
   ...
   var polarize = function (_p1) {
      var _p2 = _p1;
      var _p5 = _p2.velocity;
      var _p4 = _p2.position;
      var _p3 = $Basics.toPolar({ctor: "_Tuple2",_0: _p4.x,_1: _p4.y});
      var r = _p3._0;
      var phi = _p3._1;
      return {position: {x: r,y: phi},velocity: {x: (_p5.x * _p4.x + _p5.y * _p4.y) / r,y: (_p5.y * _p4.x - _p5.x * _p4.y) / Math.pow(r,2)},mass: _p2.mass};
   };
   var scale = F2(function (c,v) {    return {x: v.x * c,y: v.y * c};});
   var subtract = F2(function (a,b) {    return {x: a.x - b.x,y: a.y - b.y};});
   var recenter = F2(function (origin,object) {
      return _U.update(object,{position: A2(subtract,object.position,origin.position),velocity: A2(subtract,object.velocity,origin.velocity)});
   });
   var add = F2(function (a,b) {    return {x: a.x + b.x,y: a.y + b.y};});
   var nudge = F3(function (dt,derivative,state) {
      var combine = F2(function (_p7,_p6) {
         var _p8 = _p7;
         var _p11 = _p8._1;
         var _p9 = _p6;
         var _p10 = _p9._1;
         ...
         _p8._0,
         _U.update(_p10,{position: A2(add,_p10.position,A2(scale,dt,_p11.position)),velocity: A2(add,_p10.velocity,A2(scale,dt,_p11.velocity))}));
      });
      var particles = $Dict.fromList(A3($List.map2,combine,$Dict.toList(derivative.particles),$Dict.toList(state.particles)));
      return _U.update(state,{time: state.time + dt * derivative.time,particles: particles});
   });
   var evolve = F3(function (accel,dt,state) {
      var a = A2(stateDerivative,accel,state);
      var b = A2(stateDerivative,accel,A3(nudge,dt / 2,a,state));
      var c = A2(stateDerivative,accel,A3(nudge,dt / 2,b,state));
      var d = A2(stateDerivative,accel,A3(nudge,dt,c,state));
      return A3(nudge,dt / 6,d,A3(nudge,dt / 3,c,A3(nudge,dt / 3,b,A3(nudge,dt / 6,a,state))));
   });
   var totalMass = function (_p12) {
      return A3($Dict.foldl,F2(function (_p13,p) {    return F2(function (x,y) {    return x + y;})(p.mass);}),0,function (_) {    return _.particles;}(_p12));
   };
   var particle = F2(function (key,state) {    return A2($Dict.get,key,state.particles);});
   var initialize = function (dataDict) {    return {time: 0,particles: dataDict};};
   var vector = F2(function (x,y) {    return {x: x,y: y};});
   var centerOfMass = function (planets) {
      var defaultCenter = {mass: 0,position: A2(vector,0,0),velocity: A2(vector,0,0)};
      var divideByMass = function (center) {
         return _U.update(center,{position: A2(scale,1 / center.mass,center.position),velocity: A2(scale,1 / center.mass,center.velocity)});
      };
      var weightedSum = F2(function (planet,center) {
         return {mass: center.mass + planet.mass
                ,position: A2(add,center.position,A2(scale,planet.mass,planet.position))
                ,velocity: A2(add,center.velocity,A2(scale,planet.mass,planet.velocity))};
      });
      return divideByMass(A3($List.foldl,weightedSum,defaultCenter,planets));
   };
   var Vector = F2(function (a,b) {    return {x: a,y: b};});
   var Particle = F3(function (a,b,c) {    return {position: a,velocity: b,mass: c};});
   var State = F2(function (a,b) {    return {time: a,particles: b};});
   return _elm.Mechanics.values = {_op: _op
                                  ,evolve: evolve
                                  ,vector: vector
...
Elm.Orbits = Elm.Orbits || {};
Elm.Orbits.make = function (_elm) {
   "use strict";
   _elm.Orbits = _elm.Orbits || {};
   if (_elm.Orbits.values) return _elm.Orbits.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Mechanics = Elm.Mechanics.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var view = function (model) {
      var circle = function (_p0) {    return A2($Graphics$Collage.outlined,$Graphics$Collage.dashed($Color.grey),$Graphics$Collage.circle(_p0));};
      var reference = $Graphics$Collage.group(_U.list([circle(30),circle(90),circle(150)]));
      var planet = function (key) {
         var _p1 = A2($Mechanics.particle,key,model);
         if (_p1.ctor === "Just") {
               var _p2 = _p1._0;
               var angle = _p2.position.y;
               var vScale = 0.15;
               var radSpeed = vScale * _p2.velocity.x;
               var rotSpeed = vScale * _p2.velocity.y;
               var rScale = 0.8;
               var radius = rScale * _p2.position.x;
               var m = $Basics.sqrt(_p2.mass);
               return A2($Graphics$Collage.move,
               $Basics.fromPolar({ctor: "_Tuple2",_0: radius,_1: angle}),
               $Graphics$Collage.group(_U.list([A2($Graphics$Collage.filled,$Color.blue,$Graphics$Collage.circle(m))
                                               ,A2($Graphics$Collage.rotate,
                                               angle,
                                               A2($Graphics$Collage.traced,
                                               $Graphics$Collage.solid($Color.darkGrey),
                                               A2($Graphics$Collage.segment,
                                               {ctor: "_Tuple2",_0: 0,_1: 0},
                                               {ctor: "_Tuple2",_0: radSpeed,_1: radius * rotSpeed})))])));
            } else {
               return $Graphics$Collage.group(_U.list([]));
            }
      };
      return $Html.fromElement(A3($Graphics$Collage.collage,400,400,_U.list([reference,planet("planetA"),planet("planetB")])));
   };
   var evolve = F2(function (dt,model) {
      var totMass = $Mechanics.totalMass(model);
      var gravity = 500000.0;
      var accel = F2(function (particle,s) {
         var rotSpeed = particle.velocity.y;
         var radSpeed = particle.velocity.x;
         var radius = particle.position.x;
         var invMass = totMass - particle.mass;
         var separation = radius * totMass / invMass;
         return A2($Mechanics.vector,radius * Math.pow(rotSpeed,2) - 2 * gravity * invMass * Math.pow(separation,-2),-2 * radSpeed * rotSpeed / radius);
      });
      var acceleration = $Dict.fromList(_U.list([{ctor: "_Tuple2",_0: "planetA",_1: accel},{ctor: "_Tuple2",_0: "planetB",_1: accel}]));
      return A3($Mechanics.evolve,acceleration,dt,model);
   });
   var orbital = function (planets) {
      var origin = $Mechanics.centerOfMass($Dict.values(planets));
      return $Mechanics.initialize(A2($Dict.map,
      function (_p3) {
         return function (_p4) {
            return $Mechanics.polarize(A2($Mechanics.recenter,origin,_p4));
         };
      },
      planets));
   };
   var init = A2($Debug.log,
   "init",
   orbital($Dict.fromList(_U.list([{ctor: "_Tuple2",_0: "planetA",_1: {position: A2($Mechanics.vector,0,0),velocity: A2($Mechanics.vector,0,40),mass: 50}}
                                  ,{ctor: "_Tuple2"
                                   ,_0: "planetB"
                                   ,_1: {position: A2($Mechanics.vector,-200,0),velocity: A2($Mechanics.vector,0,-400),mass: 5}}]))));
   return _elm.Orbits.values = {_op: _op,init: init,evolve: evolve,view: view};
};
Elm.PhysicsFrame = Elm.PhysicsFrame || {};
Elm.PhysicsFrame.make = function (_elm) {
   "use strict";
   _elm.PhysicsFrame = _elm.PhysicsFrame || {};
   if (_elm.PhysicsFrame.values) return _elm.PhysicsFrame.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Orbits = Elm.Orbits.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var tick = F2(function (clockTime,model) {
      var withUpdatedTime = _U.update(model,{prevClockTime: $Maybe.Just(clockTime)});
      var _p0 = model.prevClockTime;
      if (_p0.ctor === "Just") {
            var dt = $Time.inSeconds(clockTime - _p0._0);
            return _U.update(withUpdatedTime,{state: A2($Orbits.evolve,dt,model.state)});
         } else {
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([$Orbits.view(model.state)
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Start)]),_U.list([$Html.text("Play")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Pause)]),_U.list([$Html.text("Pause")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Reset)]),_U.list([$Html.text("Reset")]))]));
   });
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var init = A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),
   {state: $Orbits.init,prevClockTime: $Maybe.Nothing,isPaused: true},
   $Effects.none);
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "Tick": return model.isPaused ? {ctor: "_Tuple2",_0: model,_1: $Effects.none} : {ctor: "_Tuple2",_0: A2(tick,_p1._0,model),_1: $Effects.tick(Tick)};
         case "Start": return {ctor: "_Tuple2",_0: _U.update(model,{isPaused: false}),_1: $Effects.tick(Tick)};
         case "Pause": return {ctor: "_Tuple2",_0: _U.update(model,{isPaused: true,prevClockTime: $Maybe.Nothing}),_1: $Effects.none};
         default: return init;}
   });
   var Model = F3(function (a,b,c) {    return {state: a,prevClockTime: b,isPaused: c};});
   return _elm.PhysicsFrame.values = {_op: _op,init: init,update: update,view: view,Model: Model};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $PhysicsFrame = Elm.PhysicsFrame.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var figure = F2(function (content,label) {
      return A2($Html.article,
      _U.list([$Html$Attributes.$class("figure")]),
      _U.list([content,A2($Html.div,_U.list([$Html$Attributes.$class("figure-label")]),_U.list([$Markdown.toHtml(label)]))]));
   });
   var view = F2(function (address,model) {    return A2(figure,A2($PhysicsFrame.view,address,model),description);});
   var app = $StartApp.start({init: $PhysicsFrame.init,update: $PhysicsFrame.update,view: view,inputs: _U.list([])});
   var main = app.html;
